...
- **`AlarmReceiver`** (`BroadcastReceiver`) — fired by `AlarmManager` when due. Reads `EXTRA_INDEX` and `EXTRA_RERUN`. On the original trigger (`EXTRA_RERUN` unset) it calls `AlarmScheduler.onFired(index)` (re-arm if repeating, disable if one-shot); the automatic re-ring (`EXTRA_RERUN=1`) skips that so it never touches scheduling. Either way it starts **`AlarmService`** via `ContextCompat.startForegroundService`, forwarding both extras. It does **not** `startActivity` directly — a background broadcast can't reliably launch an activity on modern Android, which is why alarms didn't fire when the app was closed.
- **`AlarmService`** (foreground `Service`) — owns the actual ringing so it works even when the app has been swiped away. On `ACTION_START` it `startForeground`s (type `mediaPlayback`) with a high-importance, `CATEGORY_ALARM` notification whose **full-screen intent** launches `AlarmRingActivity` over the lock screen (the sanctioned background-launch path; the `ALARM_MANAGER_ALARM_CLOCK` temp-allowlist from the alarm broadcast permits the FGS start). It plays the bundled alarm ringtone (`res/raw/alarm_ringtone.mp3`, on the `USAGE_ALARM` stream, looped on API 28+) and vibrates. **Auto-timeout / re-ring lives here, not in the UI:** if undismissed it stops after `AlarmScheduler.RING_TIMEOUT_MS` (1 min); the first ring arms one re-ring 1 min later via `AlarmScheduler.scheduleRerun`, and the re-ring (`rerun` flag) stops for good after its own minute — pattern: ring 1 min → silent 1 min → ring 1 min → quiet until the next trigger. `ACTION_DISMISS` (from the ring screen) calls `AlarmScheduler.cancelRerun` and stops. When it stops it broadcasts `AlarmRingActivity.ACTION_FINISH` so the ring screen closes.
- **`AlarmRingActivity`** — thin full-screen UI, launched by the service's full-screen intent. Declared in the manifest with `showWhenLocked`/`turnScreenOn`. It no longer plays sound; "Dismiss" (`Спри`) sends `AlarmService.ACTION_DISMISS` (with the index) then `finish()`, and a registered receiver for `ACTION_FINISH` closes it when the service ends the alarm (e.g. on timeout). `singleInstance`, so a re-ring reuses it via `onNewIntent`. Needs the `USE_FULL_SCREEN_INTENT` permission (declared in the manifest).
...
Default resources in `res/values/` are **English** (the base language); `res/values-bg/` holds the **Bulgarian** translation, and the app forces `"bg"` at runtime so Bulgarian always shows. `app_name` is the Bulgarian brand name **"Аларма 2"** in every locale (it's the launcher label, which the system resolves by device locale, so it's kept constant). Day letters come from the `day_letters` string-array (`М Т В Т Ф С С` in English, `П В С Ч П С Н` in Bulgarian), read Monday-first to match the weekday mask.

## Conventions & gotchas

- The launcher icon is a **vector drawable** (`res/drawable/ic_launcher.xml`), not the usual mipmap PNGs — the only binary asset is the alarm sound `res/raw/alarm_ringtone.mp3`.
